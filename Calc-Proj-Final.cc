...
class special_Hash_Table {
  public:
	...
	int return_Variable_Value(char variable, bool inc) {
  ...
  public:
	...
	int pop_Back() {
		...
		if (nums.back()[0].size() > 1)
			...
			val = Map.return_Variable_Value(nums.back()[0][0], true);
		nums.pop_back();
		return val;
	}
};
...
int do_operations(number_Deque NumDeque, char_Deque CharDeque) {
	if (NumDeque.nums.size() <= CharDeque.chars.size()) die();
			...
			if (MyNumbers.nums.size() == MyChars.chars.size()) {
				...
				if (is_A_Number(vec.at(i))) { //If it's a number.
					...
					if (Map.no_Duplicates(vec[i][0])) //If it doesn't exist then it can't be accessed.
						die();
					if (vec.size() == 1)
						val = Map.return_Variable_Value(vec[i][0], true);
					else val = Map.return_Variable_Value(vec[i][0], false);
